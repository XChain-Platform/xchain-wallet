...
| `publish.sh` | §6 step 5: upload a signed release to the feed, channel pointers last, with an edge check between the two and a cache purge after. `--staging` also takes the android partial release as signed and writes its `android/latest.json` last, which is what `rehearse.mjs run --lane android-direct` reads. | Live (host pending) |
...
| `rehearsal-matrix.mjs` | The shipped update lanes and the named hardware each is smoked on (DD4). Two sets: `LANES` (electron-updater) and `DIRECT_LANES` (the sideloaded APK, whose feed carries a notice and no installer). Data, not code; `--direct-lanes-for` answers publish.sh's question of which direct lanes a partial release's lanes ship for. | Live |
...
| `cws-upload.mjs` | D4: upload a signed release zip to the Chrome Web Store through the store's own API. Every safeguard in it is a refusal, because a refresh token that can publish to the store IS the publisher account: credentials come from the environment and no path can print one, it refuses a zip no signed manifest describes, it requires `--tag` and refuses a manifest that belongs to another release (a stale but genuinely signed pair passes every other check, because it agrees with itself), and it refuses to publish publicly without being told twice. | Live, and driven against the real staged artifacts, where it refuses correctly (no signed manifest exists yet) |
...
4c. Rehearse the update against the staging feed (§7.5). Publish
   ...
   **The direct Android lane is probed by the same command**,
   ...
   An Android release is a partial one (`sign.sh --lane android`), and it is
   staged AS SIGNED: the APK has no rehearsal twin, because its client
   hard-codes the production feed URL. `publish.sh --staging` takes that
   production directory, uploads the APK and its manifest, and writes
   `android/latest.json` naming the tag last:
   ...
   Step 5 then refuses that release without the record, because its lane
   ships the APK: the §7.5 waiver covers only lanes with no electron-updater
   feed AND no artifact a direct lane distributes. Run it without
   `--pinned-key`; a record made against any key but K1 does not clear
   `assert`.
...
- ⏸ The swap half of the rehearsal has never been observed on any lane. DD4 is answered for all eight desktop lanes: every entry in `rehearsal-matrix.mjs` names the device its swap is watched on, and `rehearsal.smoke.js` refuses a lane with `device: null`. Naming a device is not a rehearsal, so `rehearse.mjs coverage` still reports each lane as `device <name>, never rehearsed` and exits 1 until every shipped lane has an attested swap; the matrix names K1 as what blocks that now.
- ⏸ The direct Android lane's INSTALL-OVER is blocked on the same kind of open question, DD-A: no device is named, so `attest` refuses it and `assert` prints `unrehearsed, not proven` on every publish that carries an APK, the android partial release included. Its feed and client halves are rehearsed per release by `rehearse.mjs run --lane android-direct` against the staged partial (step 4c), and `publish.sh` refuses the production publish without that record. The machinery is driven end to end, staging included, by `test/smoke/audits/publish-feed.smoke.js` and `android-update-rehearsal.smoke.js` against a throwaway key; no release has yet been rehearsed this way against K1. Naming a device in `rehearsal-matrix.mjs` is what turns the note into a requirement; nothing else changes.
