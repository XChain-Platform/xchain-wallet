...
//
// That is the state the Android bundle was in, and the control
...
// are one claim split across two files in two repos, known to drift silently,
// and the cost of that drift shows up once the
...
export function runChecks({
    ...
    if (!entitlements) {
        ...
        // The Team ID half of the same string, which nothing read before
        // this: `<TEAM>.<bundle id>` is one appID and the suffix check
...
// three copies of this arithmetic used to disagree before being reconciled into one.
