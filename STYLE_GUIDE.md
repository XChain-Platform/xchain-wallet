...
- If a chain has no oracle, FeeSelector renders its own placeholder ("Fee estimate unavailable for this chain.") — don't hide the section, leave it visible so the affordance is discoverable.
...
- The icon image renders with `object-fit: contain` and **no `border-radius`** on the image itself — let the published art keep its shape (round / square / odd). Any focus ring goes on the wrapper, not the image.
...
- The handler should always navigate to the picker (clear prefill, push `'<route>-picker'`). This stays true regardless of how the user arrived at the screen — discoverable single-purpose action.
