...
/**
 * How long the explorer gets to publish the mempool row before CLAIM 2 calls
 * it a venue failure.
 *
 * The measured worst case from broadcast to first sighting is ~85s (spec §1,
 * "Timing floor"). This is 2.5x that, which is the same shape as M2.2's own
 * 180s `NETWORK_SEEN_WINDOW_MS` and leaves room for a decoder that is a poll
 * behind on a shared venue without leaving room for a decoder that is simply
 * not working.
 */
const MEMPOOL_SIGHTING_BUDGET_MS = 210_000;
...
/**
 * How many blocks this venue's miner has produced, ever.
 *
 ...
 * after the mint's mining flurry as the indexer catches up, so an assertion
 * built on it would fail on a perfectly parked miner. The miner's own counter
 * is the direct measurement, it also counts explicit `generate_blocks` calls
 * (which is what makes it useful - a neighbour or the price keeper mining into
 * this window is exactly the interference worth catching), and nothing else on
 * a regtest chain produces blocks.
 */
async function blocksMined() {
 ...
 *
 * The independent half of CLAIM 2: the wallet saying "pending" is the wallet
 * reporting on itself, and this is the venue reporting that a node is holding
 * the same transaction. Deliberately reads the unfiltered `/api/mempool`
 * window rather than the address-filtered form the wallet uses, so a failure
 * here means "no mempool row exists" rather than "the address prefilter missed
 * it", which are different bugs in different repos.
 */
async function waitForMempoolRow(txid, timeoutMs = MEMPOOL_SIGHTING_BUDGET_MS) {
 ...
 *
 * `waitForValidAction` has already established the action is indexed and valid
 * by the time this is called, so anything left here is the address mapping
 * catching up and mining at it would only put the decoder further behind.
 */
...
function pendingRowFor(page, txid) {
...
function confirmedRowFor(page, actionIndex, address) {
 ...
 * confirmed one, so it is the only search term that can ask "how many rows does
 * this transaction have" across the confirmation boundary. Everything else
 * about the entry (its key, its action index, its block) changes underneath.
 *
 * Note there is NO date widening here, deliberately. See the header.
 */
async function searchForTx(page, txid) {
