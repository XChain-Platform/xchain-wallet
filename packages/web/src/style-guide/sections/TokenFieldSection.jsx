...
export function TokenFieldSection() {
    ...
    return (
        <Section
            ...
            kicker="The compact 'which token' field for forms, the token sibling of the To / From address fields: a label above a standard field showing the selected token's icon, chain pip, ticker, and chain (or 'Select a token'), with a trailing token-list icon. Tapping it opens the token list (the same balances + search + network toggle screen Send uses). Replaces the bare 'type the ticker' text input used across the action forms."
        >
            <Guidance
                what={<>A field-shaped button showing <code>TickerIcon</code> (token icon + chain pip) + ticker + "on {'{chain}'}" + a trailing token-list icon (rows with thumbnails) in the AddressField icon slot, styled to match the <code>Input</code> chrome at the same <code>size</code>. Presentation + click only: it calls <code>onOpenPicker</code> and the host opens the full-page <code>TokenPicker</code> and feeds the chosen <code>{'{ chainId, tick }'}</code> back via <code>value</code> (the same pattern as <code>AmountField</code>, which is presentation-only while the route owns the data).</>}
