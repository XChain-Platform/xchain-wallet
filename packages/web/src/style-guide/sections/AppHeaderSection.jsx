...
export function AppHeaderSection() {
    ...
    return (
        ...
        >
            <Guidance
                what={<>A flex row with the logo (58px wide, aspect-ratio preserved) on the left and a stack of right-side action buttons. Background is <code>--xc-surface</code> with a bottom hairline. Each right-side button is a 38×38 outlined square (1px <code>--xc-border-strong</code>, 8px radius) — uniform shape lets the user scan the strip without reading it.</>}
                when={<>Mounted by the layout wrapper (FullLayoutWithNav), never by individual routes. Stays visible on every unlocked screen. Route screens render the secondary toolbar (<code>ScreenHeader</code>) below it; the AppHeader is the higher-order chrome.</>}
