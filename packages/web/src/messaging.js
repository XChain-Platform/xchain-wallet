 ...
 * fee + ADS, and runs the tamper check host-side. Resolves with a
 * serializable, already-tamper-verified HostComposeEnvelope; rejects (tamper /
 * compose failure) so the invoking form renders the error and no modal opens.
 *
...
/**
 * Run sdk.preflight host-side and return the report. `bypassCache`
 * powers the Approve-time staleness re-check.
 *
...
/**
 * Persist the in-flight confirm so a tab CLOSE costs a tap instead of
 ...
 * no confirm is ever resumed on web or mobile. A refresh re-locks the wallet
 * anyway. Kept for surface parity: shared routes call it by name on every shell.
 *
 ...
 * @returns {Promise<{ payload: object, encoded: string }>}
 */
 ...
 * @returns {Promise<{ verification: object, partnerPairing: object }>}
 */
 ...
 * on the encrypted seed, per the §26 password-never-stored posture). When
 * `opts.signerId` names a paired hardware signer (§17.6 / G023), the
