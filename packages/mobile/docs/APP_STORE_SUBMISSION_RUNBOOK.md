...
| 0h | Support URL | ✅ **BUILT 2026-08-03: `https://xchain.io/wallet/support/`** (xchain-websites, in the sitemap and llms.txt with its own OG card). **Keep the correction this row carried, because it was right and a concurrent session had built on the wrong premise:** `support@xchain.io` does NOT wait on.  is the OUTBOUND relay on origin-host and has nothing to do with receiving; `dig MX` puts both domains on Google Workspace, re-measured 2026-08-03. So a mailbox was never blocked. **A page is still the better answer to this field**, which is why it was built rather than an address published: it carries the seed-phrase scam warnings, the bug-report route and the private security route, and a bare address on a wallet's store listing is scraped by phishers within days. `info@dankest.llc` can be added to the page later, once someone has driven a message to it end to end; MX records do not prove a specific address has a mailbox |
...
| App icon (1024×1024 marketing icon) | `ios/App/App/Assets.xcassets/AppIcon.appiconset/`, regenerated from the vector mark by. **The PRIMARY icon must carry no alpha channel; the dark and tinted variants must keep theirs.** That asymmetry is the trap, and it cuts both ways: an opaque primary is Apple's rule (a transparent one is rejected at upload, before a reviewer sees it), while the dark and tinted appearances are drawn against the system's own backdrop and need transparency to work. Measured 2026-08-03, all three at 1024×1024: `AppIcon-512@2x.png` alpha=no, `AppIcon-dark-1024.png` alpha=yes, `AppIcon-tinted-1024.png` alpha=yes. That is the correct state - do not "fix" the two that have alpha. The set also ships INSIDE the ipa, so unlike every other row in this table a correction costs a new build and a new build number, not a console edit |

The iPad set is **mandatory** for a universal app. A missing iPad set blocks
submission; it does not degrade.

The app icon is in this table for a reason the others are not: it is the one
listing asset that is compiled into the binary. Everything else here can be
fixed in the console after an upload; the icon cannot. Confirm it is the
XChain mark and not the Capacitor template before Phase 3 archives, because
that is the last cheap moment ( found the template art shipping on
both shells, and no gate caught it because every gate here checks wiring and
...
| Decision | Blocks |
|---|---|
| ~~D7 territories~~ | **DECIDED 2026-08-02** (operator: mirror Android D8 exactly), see Phase 0f. Struck for the same reason as the row below rather than deleted |
| D3 external TestFlight | Phase 5's shape, and the review history D2 wanted |
| The subtitle, one of three candidates | Phase 6. `APP_STORE_LISTING.md` offers three that fit Apple's 30-character limit and defers the pick to the console. Picking it here costs nothing; improvising it there is how a listing acquires a subtitle nobody chose |
| "Mac (Designed for iPhone/iPad)" availability | Phase 4 step 2.  §9 D1 **recommends** opting out (an untested WKWebView wallet on macOS beside the Electron one, two vaults, two update paths) and was never actually answered, so that step currently assumes a decision the spec did not get |
| ~~ the deployed privacy text~~ | **RESOLVED 2026-08-02**, see Phase 0d. Left as a struck row rather than deleted because this table and Phase 0d disagreed for a day, and the table is the shorter read |
